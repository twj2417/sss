...
def scatter_fraction(config,sinogram,lors):
    ...
    scatter = np.zeros((int(scanner.nb_detectors*(scanner.nb_detectors-1)/2),1),dtype=np.float32)
    return loop_all_lors(scanner.nb_detectors_per_ring,scanner.nb_blocks_per_ring,np.array(scanner.blocks[0].grid,dtype=np.int32),
        np.array(scanner.blocks[0].size,dtype=np.int32),scatter_position,crystal_position,config['energy']['window'][0],config['energy']['window'][1],
        config['energy']['resolution'],atten,sumup_emission,num_scatter,u_map.data,u_map.size,u_map.data.shape,index,lors)


@cuda.jit
def loop_all_lors(nb_detectors_per_ring,nb_blocks_per_ring,grid_block,size_block,scatter_position,crystal_position,low_energy,high_energy,resolution,atten,sumup_emission,num_scatter,u_map,size,shape,sinogram,lors,scatter_ab):
