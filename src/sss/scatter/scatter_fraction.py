...
def scatter_fraction(config,index,lors,scanner):
    ...
    loop_all_lors[(512,512),(16,16)](scanner.nb_blocks_per_ring*scanner.blocks.shape[1],scanner.nb_blocks_per_ring,np.array(scanner.blocks.shape,dtype=np.int32),
    ...
    scale = scale*4*math.pi/efficiency_without_scatter**2/(scanner.blocks.size[1]*scanner.blocks.size[2]/scanner.blocks.shape[1]/scanner.blocks.shape[2])**2
